...
class vicsekmodel:
    def __init__(self, N, timesteps, discard, repeat, Lx, Ly, save_interval, boundary, imflag=False, imflag2=False, plotflag=False, disable_progress=False):
        ...
        self.macro_state3 = np.zeros((self.B, self.timesteps - self.discard))
    ...
    def initialise_state(self):
        ...
        self.tot_neigh = np.zeros((self.B,self.N,1)) #array to store information on number of neighbours
        self.X_left = np.zeros((self.B, 1))
        self.X_right = np.zeros((self.B, 1))
    ...
    def run_sim(self, eta, radius, speed, pressure_change, diss, l0, attraction, flow_align_effect, aspect, mu, force):
        ...
        def update(X, A):
            ...
            if self.imflag == True:
                ...
                #stokes_flow = np.zeros((self.B, 2*self.N, 2))
            ...
            """for b in range(self.B):
                for i in range(self.N):
                    stokesflow[b,i,:] = stokes_flow(b, i, pos_vectors, dist, mu, stokes_force[b,i,:])"""
            ...
            if self.boundary_conditions == "periodic":
                ...
                X_left, X_right = np.count_nonzero(np.where(X[...,0:1]<0, 1, 0), axis=1), np.count_nonzero(np.where(X[...,0:1]>self.Lx, 1, 0), axis=1)
    ...
    def get_macro_states(self):
        ...
        macro_t = np.ravel(self.macro_state3[:,:-1])
        
        return macro_x, macro_dx, macro_g, macro_t